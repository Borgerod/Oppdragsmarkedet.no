import { pgTable, index, foreignKey, unique, check, text, varchar, json, timestamp, integer, boolean, date, real, point, pgEnum } from "drizzle-orm/pg-core"
...
export const userRole = pgEnum("user_role", ['private', 'business', 'government', 'employee', 'admin'])
...
export const favoriteProjects = pgTable("favorite_projects", {
	...
	check("favorite_projects_url_check", sql`(url)::text ~* '^https?://(www\.)?oppdragsmarkedet\.no/oppdrag/\?prosjekt_id=pro[^\s]*$'::text)) NOT VALID`),
]);
...
export const user = pgTable("user", {
	...
	check("users_username_check", sql`(length(username) >= 3) AND (username ~ '^[a-zA-Z0-9_]+$'::text)`),
