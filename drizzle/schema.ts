...
export const favoriteProjects = pgTable(
	'favorite_projects',
	{
	...
	(table) => [
		...
		check(
			...
			sql`(url)::text ~* '^https?://(www\.)?oppdragsmarkedet\.no/oppdrag/\?prosjekt_id=pro[^\s]*$'::text)) NOT VALID`
		)
	]
);
...
export const savedFilters = pgTable(
	...
	(table) => [
		...
		check(
			'saved_filters_url_check',
			sql`(url)::text ~* '^https?://(www\.)?oppdragsmarkedet\.no(/.*)?$'::text`
		)
	]
);

export const followedClients = pgTable(
	'followed_clients',
	{
		id: text().primaryKey().notNull(),
		userId: text('user_id').notNull(),
...
export const socialMedia = pgTable(
	...
	(table) => [
		...
		check(
			'social_media_facebook_check',
			sql`(facebook)::text ~* '^https?://(www.)?facebook.com(/.*)?$'::text`
		),
		check(
			'social_media_tiktok_check',
			sql`(tiktok)::text ~* '^https?://(www.)?tiktok.com(/.*)?$'::text`
		),
		check(
			'social_media_linkedin_check',
			sql`(linkedin)::text ~* '^https?://(www.)?linkedin.com(/.*)?$'::text`
		),
		check(
			'social_media_twitter_check',
			sql`(twitter)::text ~* '^https?://(www.)?twitter.com(/.*)?$'::text`
		),
		check(
			'social_media_instagram_check',
			sql`(instagram)::text ~* '^https?://(www.)?instagram.com(/.*)?$'::text`
		),
		check(
			'social_media_homepage_check',
			sql`(homepage)::text ~* '^https?://(www.)?[-a-zA-Z0-9@:%._+~#=]{1,256}.[a-zA-Z]{2,6}(/.*)?$'::text`
		),
		check(
			'social_media_proff_check',
			sql`(proff)::text ~* '^https?://(www.)?[-a-zA-Z0-9@:%._+~#=]{1,256}.[a-zA-Z]{2,6}(/.*)?$'::text`
		)
	]
);
...
export const projects = pgTable(
	'projects',
	{
		...
		state: projectStatus().default('draft'),
		...
		subCategory: varchar('sub_category', { length: 40 }),
		experienceRequirements: varchar('experience_requirements', { length: 50 }).array(),
		thumbnail: varchar({ length: 250 }),
		jobAttributes: varchar('job_attributes', { length: 50 }).array(),
		clientRole: varchar('client_role', { length: 10 }),
		imageGallery: varchar('image_gallery', { length: 250 }).array()
	},
	(table) => [
...
export const userProfiles = pgTable(
	...
	(table) => [
		...
		unique('user_profiles_id_unique').on(table.id),
...
export const user = pgTable(
	'user',
	{
		...
		image: text(),
		isThirdPartyVerified: boolean('is_third_party_verified').default(false)
	},
	(table) => [
		...
		check(
			...
			sql`(length((username)::text) >= 3) AND ((username)::text ~ '^[a-zA-Z0-9_]+$'::text)`
		),
		check(
			'users_email_check',
			sql`(email)::text ~* '^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$'::text`
		)
	]
);
