...
<Drawer.Root>
	<Drawer.Trigger>Filter</Drawer.Trigger>
	<Drawer.Portal>
		<Drawer.Overlay class="fixed inset-0 bg-black/40" />
		<Drawer.Content
			class="fixed right-0 bottom-0 left-0 mt-24 flex h-[96%] flex-col rounded-t-[10px] bg-zinc-100"
		>
			<div class="flex-1 rounded-t-[10px] bg-white p-4" style="background-color: red ">
			...
			<div class="mt-auto border-t border-zinc-200 bg-zinc-100 p-4">
				<div class="mx-auto flex max-w-md justify-end gap-6">
					<a
						class="flex items-center gap-0.25 text-xs text-zinc-600"
